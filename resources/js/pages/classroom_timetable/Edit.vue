<template>
	<v-card class="mx-auto p-4" width="700" raised>
	  ...
	  <v-spacer></v-spacer>
	  		<v-select
				v-model="classroom_timetable.subject"
				:items="subjects"
				item-text="title"
				item-value="id"
				menu-props="auto"
...
<script>
  export default {
    ...
    created() {
        let url = process.env.MIX_APP_URL + `/api/classroom_timetable/${this.$route.params.id}/edit`;
        this.axios.get(url).then((response) => {
            this.classroom_timetable = response.data;
        });
      },
      mounted() {
		this.getDepartments();
    ...
    methods: {
		...
		getSubjects() {
			axios.get("classrooms/{classroom_id}/subjects").then(
				res => {
					console.log(res.data);
					this.departments = res.data;
				},
        ...
        submit() {
        this.errors = {};
        axios.put(`/batches/${this.$route.params.id}/update/`, this.batches).then(response => {
